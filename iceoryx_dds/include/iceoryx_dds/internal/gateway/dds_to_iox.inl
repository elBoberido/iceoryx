                ...
                .and_then([&](auto userPayload) {
                    ...
                    reader
                        ...
                        .or_else([&](DataReaderError err) {
                            ...
                            IOX_LOG(WARN) << "[DDS2IceoryxGateway] Encountered error reading from DDS network: "
                                          << dds::DataReaderErrorString[static_cast<uint8_t>(err)];
                        });
                })
                .or_else([](auto& error) {
                    IOX_LOG(ERROR) << "[DDS2IceoryxGateway] Could not loan chunk! Error code: "
                                                       ...
                                                       const popo::PublisherOptions& publisherOptions) noexcept
{
    return this->addChannel(service, publisherOptions).and_then([&service](auto channel) {
        ...
        publisher->offer();
        reader->connect();
        IOX_LOG(DEBUG) << "[DDS2IceoryxGateway] Setup channel for service: {" << service.getServiceIDString() << ", "
